...
import {closeErrorMessage} from "./TextToSearchSlice";

export function TextToSearchErrorDialog() {
    const dispatch = useDispatch();

    const show_error_form = useSelector((state) => state.textToSearchReducer.show_error_form);
    const error_title = useSelector((state) => state.textToSearchReducer.error_title);
    const error_message = useSelector((state) => state.textToSearchReducer.error_message);
