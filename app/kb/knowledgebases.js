...
export class KnowledgeBases extends React.Component {
    ...
    changePage(page) {
        ...
        const knowledge_base_list = this.kba.knowledge_base_list;
        const nav_list = this.state.nav_list;
        if (page0 !== page && page >= 0) {
            if (page0 < page) {
                // next page
                if (knowledge_base_list.length > 0) {
                    const prev = knowledge_base_list[knowledge_base_list.length - 1].kbId;
                    nav_list.push(prev);
                    this.setState({nav_list: nav_list, page: page});
                    this.kba.refreshKnowledgeBaseList(prev, this.state.page_size);
                }
            } else {
                // prev page
    ...
    changePageSize(page_size) {
        ...
        this.kba.refreshKnowledgeBaseList(this.state.prev_page, page_size);
    }
    ...
    render() {
        if (this.state.has_error) {
            return <h1>knowledgebases.js: Something went wrong.</h1>;
        }
        return (
            <div>
                <ErrorDialog
                ...
                <Paper>
                    <Table>
                        ...
                        <TableBody>
                            {
                                this.kba.knowledge_base_list.map((knowledge_base) => {
                                    return (
                    ...
                    <TablePagination
                        ...
                        component="div"
                        count={this.kba.knowledge_base_list.length + 1}
